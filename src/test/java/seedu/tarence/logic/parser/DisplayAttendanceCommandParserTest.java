...
public class DisplayAttendanceCommandParserTest {
    ...
    public void parse_invalidArgs_returnsListCommand() {
        ...
        String invalidTutorialIndex1 = "sa&";

        // Parsing nothing
        String nullString = "";
        assertParseFailure(parser, nullString,
                invalidIndexMessage);

        // Invalid index - non-integer
        assertParseFailure(parser, invalidTutorialIndex1,
                invalidIndexMessage);
        ...
        assertParseFailure(parser, invalidTutorial,
                invalidIndexMessage);

        // Parsing invalid module
        String invalidModule = PREFIX_MODULE + "CSCS";
        assertParseFailure(parser, invalidModule,
                invalidIndexMessage);

        // Parsing invalid tutorial and module
        assertParseFailure(parser, invalidModule + invalidTutorial,
                invalidIndexMessage);
